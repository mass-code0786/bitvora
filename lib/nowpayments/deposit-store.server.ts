...
const toRecord=(row:{id:string;paymentId:string;userId:string;orderId:string;idempotencyKey:string;usdtAmount:{toNumber():number};requestedAmount:{toNumber():number};providerPayAmount:{toNumber():number};walletAddress:string;txHash:string|null;status:string;providerStatus:string;credited:boolean;expiresAt:Date|null;confirmedAt:Date|null;createdAt:Date;updatedAt:Date;user:{uid:string}}):DemoDepositRecord=>({
  ...
  requestedAmount:row.requestedAmount.toNumber(),payAmount:row.providerPayAmount.toNumber(),payCurrency:"usdtbsc",network:"USDT_BEP20",
...
export async function saveServerDeposit(record:DemoDepositRecord){if(testMode()){const index=testRecords.findIndex(item=>item.id===record.id);if(index>=0)testRecords[index]=record;else testRecords.push(record);return record}const row=await prisma.nowPaymentsDeposit.upsert({where:{paymentId:record.providerPaymentId},create:{id:record.id,paymentId:record.providerPaymentId,userId:record.userId,orderId:record.orderId,idempotencyKey:record.idempotencyKey,usdtAmount:record.actuallyPaid,requestedAmount:record.requestedAmount,providerPayAmount:record.payAmount,walletAddress:record.payAddress,txHash:record.txHash,status:storedStatus(record),providerStatus:record.paymentStatus,expiresAt:record.expiresAt?new Date(record.expiresAt):null,confirmedAt:record.finishedAt?new Date(record.finishedAt):null,createdAt:new Date(record.createdAt)},update:{usdtAmount:record.actuallyPaid,providerPayAmount:record.payAmount,txHash:record.txHash,status:storedStatus(record),providerStatus:record.paymentStatus,expiresAt:record.expiresAt?new Date(record.expiresAt):null,confirmedAt:record.finishedAt?new Date(record.finishedAt):null},include});return toRecord(row)}
...
export async function saveReconciledDeposit(record:DemoDepositRecord){
  ...
  return prisma.$transaction(async tx=>{
    ...
    await tx.nowPaymentsDeposit.update({where:{paymentId:record.providerPaymentId},data:{usdtAmount:record.actuallyPaid,providerPayAmount:record.payAmount,txHash:record.txHash,status:storedStatus(record),providerStatus:record.paymentStatus,expiresAt:record.expiresAt?new Date(record.expiresAt):null,confirmedAt:completed?new Date(record.finishedAt??Date.now()):null}});
    if(completed){
