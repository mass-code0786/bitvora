...
export function buildFirstTransferPayoutPlan(source:DbUser,users:readonly DbUser[],amount:number):PayoutPlan[]{
  ...
  for(let position=0;position<6&&sponsorId;position++){
    ...
    const direct=position===0,level=direct?0:position+1,percentage=direct?5:1;
...
export async function distributeFirstSpotToFuture(tx:Prisma.TransactionClient,input:{source:DbUser;users:DbUser[];amount:number;transferId:string;transferReference:string;idempotencyKey:string;timestamp:number}){
  ...
  const completedLevels=new Set(existing?.entries.map(entry=>entry.level)??[]);
  for(const payout of plan){
    if(completedLevels.has(payout.level))continue;
    ...
    if(payout.level>=2&&wallet.totalFuturePrincipal<50)continue;
    const ledgerId=`${distribution.id}:level-${payout.level}`,before=wallet.wallets.spot.balance,after=addMoney(before,payout.amount);
    ...
    await tx.userNotification.create({data:{userId:payout.recipient.id,title:payout.level===0?"Referral income credited":`Level ${payout.level} income credited`,message:`$${payout.amount.toFixed(2)} was credited to your Spot Wallet.`,type:payout.level===0?"REFERRAL_INCOME":"LEVEL_INCOME",reference:`referral-income:${ledgerId}`}});
    await tx.referralIncomeAuditLog.create({data:{distributionId:distribution.id,recipientUserId:payout.recipient.id,sourceReferredUserId:input.source.id,level:payout.level,action:"SPOT_WALLET_INCOME_CREDITED",ledgerTransactionId:ledgerId,metadata:{amount:payout.amount,percentage:payout.percentage,transactionType:payout.transactionType,sourceTransferId:input.transferId}}});
  }
