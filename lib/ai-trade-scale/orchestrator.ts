...
import { boundedQueueOperation,getTradeQueue,jobOptions,settlementQueue } from "./queue";
...
export const creationBullJobId=(run:EnqueueRun,userId:string)=>`ai-trade-create__${userId}__${run.localTradingDate}__${run.localSessionSlot}__${run.tradeType}`;
...
async function enqueueUser(run:EnqueueRun,userId:string){
  ...
  const jobKey=creationJobKey(run,userId),bullJobId=creationBullJobId(run,userId),existing=await prisma.aiTradeJob.findFirst({where:{OR:[{jobKey},{sessionRunId:run.id,userId,kind:"CREATE"}]},select:{id:true,status:true,jobKey:true,bullJobId:true}});
  if(existing){const recoverable=["QUEUED","FAILED"].includes(existing.status);if(recoverable){if(existing.bullJobId!==bullJobId)await prisma.aiTradeJob.update({where:{id:existing.id},data:{bullJobId}});await boundedQueueOperation(tradeQueue.add("create-trade",{dbJobId:existing.id,sessionRunId:run.id,userId},{...jobOptions,jobId:bullJobId}))}return{created:false,enqueued:recoverable,status:existing.status,jobKey:existing.jobKey,bullJobId}}
  ...
  await boundedQueueOperation(tradeQueue.add("create-trade",{dbJobId:dbJob.id,sessionRunId:run.id,userId},{...jobOptions,jobId:bullJobId}));
