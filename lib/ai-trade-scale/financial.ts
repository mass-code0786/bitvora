...
import { settlementBullJobId,tradeExecutionKey } from "./orchestrator";
import { queues } from "./config";
import { withPostgresSerializationRetry } from "./serialization-retry";
...
export async function createTradeForUser(sessionRunId:string,userId:string){return withPostgresSerializationRetry(()=>prisma.$transaction(async tx=>{
  ...
  await tx.aiTradeJob.create({data:{jobKey:`SETTLE:${trade.id}`,bullJobId:settlementBullJobId(trade.id),queueName:queues.settlement,sessionRunId,userId,tradeId:trade.id,kind:"SETTLE"}});
 ...
 },{isolationLevel:"Serializable",timeout:10_000,maxWait:5_000}),{context:"CREATE_TRADE"})}

export async function settleTrade(tradeId:string){return withPostgresSerializationRetry(()=>prisma.$transaction(async tx=>{
  ...
  const principalExists=await tx.aiWalletLedger.findUnique({where:{idempotencyKey:returnKey(trade.id)}}),profitExists=await tx.aiWalletLedger.findUnique({where:{idempotencyKey:profitKey(trade.id)}});if(principalExists&&profitExists)return tx.aiFinancialTrade.update({where:{id:trade.id},data:{status:"SETTLED",officialSettledAt:trade.sessionRun.officialSettledAt,processingSettledAt:trade.processingSettledAt??new Date(),failureCode:null}});if(principalExists||profitExists)throw new TradeJobError("PERMANENT_VALIDATION",false,"Partial settlement ledger detected; reconciliation required");
 ...
 },{isolationLevel:"Serializable",timeout:10_000,maxWait:5_000}),{context:"SETTLE_TRADE"})}
