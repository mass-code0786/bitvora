...
export async function buildSupportAccountContext(userId:string):Promise<SupportAccountContext>{
  const[user,team,kyc,financial]=await Promise.all([
    ...
    prisma.$queryRaw<TeamRow[]>`WITH RECURSIVE team AS (SELECT u."id",u."uid",ARRAY[u."id"] AS path FROM "User" u JOIN "User" root ON root."id"=${userId} WHERE u."sponsorId"=root."id" OR (u."sponsorId" IS NULL AND u."sponsorUid"=root."uid") UNION ALL SELECT u."id",u."uid",team.path||u."id" FROM "User" u JOIN team ON u."sponsorId"=team."id" OR (u."sponsorId" IS NULL AND u."sponsorUid"=team."uid") WHERE NOT u."id"=ANY(team.path)) SELECT COUNT(*)::bigint AS "totalTeam",COUNT(*) FILTER(WHERE COALESCE(w."retainedPrincipal",NULLIF(s."wallet"->>'totalFuturePrincipal','')::numeric,0)>=50)::bigint AS "qualifiedTeam",COALESCE(SUM(COALESCE(w."retainedPrincipal",NULLIF(s."wallet"->>'totalFuturePrincipal','')::numeric,0)),0) AS business FROM team LEFT JOIN "AiFinancialWallet" w ON w."userId"=team."id" LEFT JOIN "UserState" s ON s."userId"=team."id"`,
    getSafeKycForUser(userId).catch(()=>({status:"UNAVAILABLE" as const})),getWalletReadSnapshot(userId)
