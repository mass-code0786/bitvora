    ...
    for(const event of events){
      try{
        ...
        await prisma.$transaction(async tx=>{
          ...
          const target=event.eventType==="AI_PROFIT_TARGET_REACHED";await tx.userNotification.upsert({where:{reference:event.idempotencyKey},create:{userId:event.userId,reference:event.idempotencyKey,type:event.eventType,title:salary?"Salary Credited":target?"70% target completed":event.eventType==="TRADE_STARTED"?"Trade placed":event.eventType==="TRADE_SETTLED"?"Trade settled":"Profit credited",message:salary?`Your ${rank} Star salary of $${payload.amount} has been credited to your Spot Wallet.`:target?"Future to Spot transfers are now deduction free.":event.eventType==="PROFIT_CREDITED"?`${payload.amount} profit was credited.`:`AI trade ${payload.tradeId??event.aggregateId}`},update:{}});
