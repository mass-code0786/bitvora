...
export function KycModule(){
  ...
  return <div className="wallet-operation">
    ...
    {canSubmit&&<form className="wallet-form-card kyc-form" onSubmit={submit}>
      ...
      <label className="wallet-select-field"><span>Document type</span><select name="documentType"><option value="PASSPORT">Passport</option><option value="NATIONAL_ID">National ID</option><option value="DRIVING_LICENSE">Driving license</option></select></label>
      <label className="wallet-text-field"><span>Document number</span><input name="documentNumber" required/></label>
      <label className="wallet-text-field"><span>Front image · JPEG, PNG, WEBP · max 8 MB</span><input name="front" type="file" accept="image/jpeg,image/png,image/webp" required/></label>
      <label className="wallet-text-field"><span>Back image · required for National ID and Driving License</span><input name="back" type="file" accept="image/jpeg,image/png,image/webp"/></label>
      <label className="wallet-text-field"><span>Selfie image · JPEG, PNG, WEBP · max 8 MB</span><input name="selfie" type="file" accept="image/jpeg,image/png,image/webp" required/></label>
