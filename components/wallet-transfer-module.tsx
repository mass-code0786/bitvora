...
export function WalletTransferModule(){
  ...
  const submit=()=>{if(!valid){notify("Transfer unavailable",numeric>source.balance?"Insufficient wallet balance.":"Enter a valid transfer amount.");return}requestKey.current=`transfer-${crypto.randomUUID()}`;if(quote.earlyTransfer){setPending({amount:numeric,quote,remainingTargetPercentage:Math.max(0,100-metrics.unlockProgressPercentage)});return}void complete()};
