...
export function Dashboard(){const{store}=useWalletStore(),spot=store.wallets.spot.balance,future=store.wallets.future.balance,total=spot+future;return <div className="target-home">
  <section className="target-portfolio" aria-label="Portfolio summary">
    ...
    <div className="target-balance"><small>Total wealth</small><h1>{formatCurrency(total)}</h1><p>+12.8% Today</p></div>
    <div className="target-ring" aria-hidden="true">
      ...
      <div className="target-ring-center"><small>Growth</small><strong>+12.8%</strong></div><span className="target-ring-tag tag-portfolio">Portfolio</span><span className="target-ring-tag tag-profit">Profit</span>
    </div>
  ...
  <section className="target-activity"><div className="target-section-heading"><h2>Recent activity</h2><Link href="/history">See all</Link></div><div className="target-activity-list">{transactions.slice(0,3).map(tx=><article key={tx.date}><span className="target-activity-icon"><WalletCards size={15}/></span><div><strong>{tx.title}</strong><small>{tx.date}</small></div><aside><b className={tx.amount>=0?"positive":"negative"}>{formatCurrency(tx.amount,{sign:tx.amount>=0?"always":"negative-only",minimumFractionDigits:2})}</b><small>{tx.type}</small></aside></article>)}</div></section>
</div>}
