...
export function SessionRedirect(){const router=useRouter();useEffect(()=>{void fetch("/api/auth/session",{cache:"no-store"}).then(r=>r.json()).then(s=>{if(s?.user)router.replace("/home")})},[router]);return null}
