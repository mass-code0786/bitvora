...
export function AuthForm({mode}:{mode:"login"|"register"}){
 ...
 const[name,setName]=useState(""),[country,setCountry]=useState<CountryOption|null>(null),[countryError,setCountryError]=useState(""),[email,setEmail]=useState(""),[password,setPassword]=useState(""),[error,setError]=useState(""),[showPassword,setShowPassword]=useState(false),[sponsorUid,setSponsorUid]=useState(""),[referralLocked,setReferralLocked]=useState(false);
 ...
 async function submit(event:FormEvent){event.preventDefault();setError("");if(register&&!country){setCountryError("Please select your country.");return}if(register){try{const response=await fetch("/api/auth/register",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name,countryCode:country!.code,email,password,referralUid:sponsorUid||undefined})}),payload=await response.json() as {error?:string};if(!response.ok){setError(payload.error??"Registration failed.");return}}catch{setError("Registration is temporarily unavailable.");return}}const result=await signIn("credentials",{email,password,redirect:false});if(result?.error){setError("Invalid email or password.");return}for(let index=sessionStorage.length-1;index>=0;index--){const key=sessionStorage.key(index);if(key?.startsWith("bitvora-banner-dismissed:"))sessionStorage.removeItem(key)}router.push("/home");router.refresh()}
 return <main className="auth-bg min-h-screen px-5 py-8"><div className="ambient ambient-one"/><div className="auth-shell relative z-10 mx-auto max-w-md"><Logo/><div className="mt-14"><p className="eyebrow">Trade. Earn. Grow.</p><h1 className="mt-2 text-3xl font-semibold tracking-tight text-white">{register?"Create your account":"Welcome back"}</h1><p className="mt-2 text-sm text-slate-500">{register?"Start your Bitvora journey in a few steps.":"Sign in to continue to your dashboard."}</p></div><form onSubmit={submit} className="mt-7 space-y-4">
  ...
  {register&&<div className="field"><span>Country</span><CountryPicker value={country} error={countryError} onChange={value=>{setCountry(value);setCountryError("")}}/></div>}
