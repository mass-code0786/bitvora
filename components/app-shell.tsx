...
export function AppShell({children}:{children:ReactNode}) {
  const pathname=usePathname(),[menuOpen,setMenuOpen]=useState(false),[notificationsOpen,setNotificationsOpen]=useState(false),[notifications,setNotifications]=useState<HeaderNotification[]>([]),[notificationsLoading,setNotificationsLoading]=useState(false),dashboard=pathname==="/dashboard";
  ...
  return <div className="app-bg min-h-screen text-slate-300"><SessionGuard/><div className="noise-layer"/><div className="ambient ambient-one"/><div className="ambient ambient-two"/><div className="ambient ambient-three"/>
    ...
    <nav aria-label="Primary navigation" className="floating-nav fixed bottom-4 left-1/2 z-[90] isolate flex w-[calc(100%-2rem)] max-w-md -translate-x-1/2 items-center justify-around px-2 py-2 pointer-events-auto lg:bottom-6">{nav.map(({href,label,icon:Icon,primary})=>{const active=pathname===href;return <motion.div className="relative z-10 pointer-events-auto" key={href} whileTap={{scale:.88}}><Link href={href} className={`nav-item pointer-events-auto ${active?"active":""} ${primary?"nav-primary":""}`}>{active&&<motion.i layoutId="nav-active"/>}<Icon size={19}/><span>{label}</span></Link></motion.div>})}</nav>
