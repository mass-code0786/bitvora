...
export async function POST(request:Request){
  try{
    ...
    if(existing){const assistant=await prisma.chatMessage.findFirst({where:{conversationId:existing.conversationId,role:"ASSISTANT",createdAt:{gte:existing.createdAt}},orderBy:[{createdAt:"asc"},{id:"asc"}]});return NextResponse.json({session:existing.conversation,message:assistant,replayed:true})}
    ...
    const records=await prisma.supportRule.findMany({where:{isActive:true},orderBy:[{priority:"desc"},{intentKey:"asc"}]}),rules=records.map(databaseRule),matched=matchSupportRule(value.message,rules);
    let response=FALLBACK_RESPONSE,followUps=FALLBACK_TOPICS,ruleId:string|null=null,intentKey="UNMATCHED",score=0;
    if(matched){const context=await buildSupportAccountContext(user.id);response=renderSupportResponse(matched.rule.response,context);followUps=matched.rule.followUps;ruleId=matched.rule.id;intentKey=matched.rule.intentKey;score=matched.score}
    ...
    return NextResponse.json({session,message:result.assistant,matchedIntent:intentKey,matchScore:score,followUps,unmatched:!matched});
  }catch(error){
