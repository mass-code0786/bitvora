...
export async function GET(){try{await requireDemoAdmin();const records=await listKycRecords(),users=await prisma.user.findMany({select:{id:true,uid:true,name:true,email:true}}),byUid=new Map(records.map(item=>[item.userUid,item]));return NextResponse.json({records:users.map(user=>{const item=byUid.get(user.uid);return{userId:user.id,userUid:user.uid,name:user.name,email:user.email,country:item?.country??"",documentType:item?.documentType??null,maskedDocumentNumber:item?maskDocumentNumber(item.documentNumber):null,status:item?.status??"NOT_SUBMITTED",submittedAt:item?.submittedAt??null,submissionVersion:item?.submissionVersion??0,reviewedAt:item?.reviewedAt??null,reviewedBy:item?.reviewedBy??null}}).sort((a,b)=>Number(b.status==="PENDING")-Number(a.status==="PENDING")||(b.submittedAt??0)-(a.submittedAt??0))})}catch{return NextResponse.json({error:"Admin access required."},{status:401})}}
