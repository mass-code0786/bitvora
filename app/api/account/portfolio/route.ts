...
export async function GET(){
  try{
    const authUser=await requireAuthenticatedUser(),[user,financial]=await Promise.all([prisma.user.findUnique({where:{id:authUser.id},select:{timezone:true,country:true,state:{select:{wallet:true}}}}),getWalletSnapshot(authUser.id)]);
