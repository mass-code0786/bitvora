...
export async function POST(request:Request){
  try{
    ...
    const result=await prisma.$transaction(async transaction=>{
      ...
      if(alreadyProcessed){const details=wallet.transactions.find(item=>item.id===`${value.idempotencyKey}:credit`)?.transferDetails;if(!details)throw new Error("Transfer request was already processed.");if(value.from==="spot")console.info(JSON.stringify({component:"first-transfer-referral",code:referralLogCodes.alreadyExists,referredUserId:user.id,sourceTransferId:`${value.idempotencyKey}:debit`}));return{wallet,quote:{grossAmount:details.grossAmount,earlyTransferFee:details.deductionAmount,netCredit:details.netAmount,earlyTransfer:details.deductionAmount>0,progressPercentage:details.progressPercentage},alreadyProcessed:true}}
