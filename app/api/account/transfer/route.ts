...
export async function POST(request:Request){
  try{
    ...
    const result=await prisma.$transaction(async transaction=>{
      const user=await transaction.user.findUniqueOrThrow({where:{id:authUser.id},select:{id:true,uid:true,sponsorId:true,sponsorUid:true}}),state=await transaction.userState.findUnique({where:{userId:user.id}});let wallet=migrateWalletStore(state?.wallet??cloneWalletSeed());await transaction.aiFinancialWallet.upsert({where:{userId:user.id},create:{userId:user.id,availableFuture:new Prisma.Decimal(String(wallet.wallets.future.balance)),lockedFuture:new Prisma.Decimal(String(wallet.lockedFutureTradeCapital)),completedProfit:new Prisma.Decimal(String(wallet.totalCompletedTradingProfit))},update:{}});await transaction.$queryRaw`SELECT "userId" FROM "AiFinancialWallet" WHERE "userId"=${user.id} FOR UPDATE`;const financial=await transaction.aiFinancialWallet.findUniqueOrThrow({where:{userId:user.id}});wallet={...wallet,wallets:{...wallet.wallets,future:{...wallet.wallets.future,balance:Number(financial.availableFuture.toString())}},lockedFutureTradeCapital:Number(financial.lockedFuture.toString())};const alreadyProcessed=wallet.processedKeys.includes(value.idempotencyKey);
      ...
      const exactAmount=new Prisma.Decimal(String(amount)),availableAfter=value.from==="spot"?financial.availableFuture.add(exactAmount):financial.availableFuture.sub(exactAmount);if(availableAfter.isNegative())throw new Error("Insufficient Future Wallet balance.");await transaction.aiFinancialWallet.update({where:{userId:user.id},data:{availableFuture:availableAfter,version:{increment:1}}});
