...
const input=z.object({from:z.literal("future"),amount:z.number().positive(),idempotencyKey:z.string().min(8).max(120),confirmedEarlyTransfer:z.boolean().optional().default(false)});
...
export async function POST(request:Request){
  try{
    ...
    const result=await prisma.$transaction(async transaction=>{
      ...
      const quote=getTransferQuote(wallet,"future",amount);
      ...
      const next=transferBetweenWallets(wallet,"future",amount,value.idempotencyKey,Date.now()),stored=await transaction.userState.upsert({where:{userId:user.id},create:{userId:user.id,wallet:next as object,trading:createEmptyTradingStore() as object},update:{wallet:next as object}});
