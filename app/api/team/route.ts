...
export async function GET(request:Request){
  try{
    ...
    ) SELECT allowed."id",allowed."uid",allowed."name",root."uid" AS "selfUid",root."name" AS "selfName",sponsor."uid" AS "sponsorUid",sponsor."name" AS "sponsorName",root."timezone",root."country" FROM allowed CROSS JOIN "User" root LEFT JOIN "User" sponsor ON sponsor."id"=root."sponsorId" WHERE root."id"=${current.id} AND allowed."uid"=${parentUid} LIMIT 1`;
    const record=parents[0];if(!record)return NextResponse.json({error:"Genealogy branch is not available."},{status:403});
    const parent={id:record.id,uid:record.uid,name:record.name},identity={self:{uid:record.selfUid,name:record.selfName},sponsor:record.sponsorUid?{uid:record.sponsorUid,name:record.sponsorName??"—"}:null},monthStart=startOfLocalMonthUtc(Date.now(),resolveUserTimeZone(record.timezone,record.country));
    ...
    ) SELECT principals."rootId",(COUNT(*)-1)::bigint AS "totalTeam",COUNT(*) FILTER (WHERE principals."id"<>principals."rootId" AND principals."principal">=50)::bigint AS "qualifiedTeam",COUNT(*) FILTER (WHERE principals."sponsorId"=principals."rootId")::bigint AS "directCount",COUNT(*) FILTER (WHERE principals."id"<>principals."rootId" AND principals."createdAt">=${monthStart})::bigint AS "newThisMonth",COALESCE(SUM(principals."principal") FILTER (WHERE principals."id"<>principals."rootId"),0) AS "totalBusiness",COALESCE(MAX(principals."principal") FILTER (WHERE principals."id"=principals."rootId"),0) AS "personalBusiness" FROM principals GROUP BY principals."rootId"`:[],byRoot=new Map(aggregates.map(row=>[row.rootId,row]));
