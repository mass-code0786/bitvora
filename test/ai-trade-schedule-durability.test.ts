...
import { creationBullJobId,creationJobKey,scheduleSessionsForUser,settlementBullJobId,tradeExecutionKey } from "@/lib/ai-trade-scale/orchestrator";
...
describe("durable per-slot AI Bot scheduling",()=>{
  ...
  it("same-slot duplicates use one stable database and Bull job identity",()=>expect([creationJobKey(run("1"),"user"),creationJobKey(run("1"),"user"),creationBullJobId(run("1"),"user"),creationBullJobId(run("1"),"user")]).toEqual(["CREATE:user:2026-08-01:1:AI_BOT","CREATE:user:2026-08-01:1:AI_BOT","ai-trade-create__user__2026-08-01__1__AI_BOT","ai-trade-create__user__2026-08-01__1__AI_BOT"]));
  it("BullMQ job IDs include slot and contain no forbidden colon",()=>{for(const slot of ["1","2","3","ADDITIONAL"]){const id=creationBullJobId(run(slot),"user");expect(id).toContain(`__${slot}__`);expect(id).not.toContain(":")}expect(settlementBullJobId("trade")).toBe("ai-trade-settle__trade")});
  ...
  it("Redis restart recovery uses persisted queued jobs and regenerated valid IDs",()=>{const orchestrator=source("lib/ai-trade-scale/orchestrator.ts");expect(orchestrator).toContain('where:{status:"QUEUED"}');expect(orchestrator).toContain("creationBullJobId");expect(orchestrator).toContain("boundedQueueOperation")});
