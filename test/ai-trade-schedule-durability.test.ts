...
import { createBullJobId,creationJobKey,resolveRecoveryBullJobId,scheduleSessionsForUser,settlementBullJobId,tradeExecutionKey } from "@/lib/ai-trade-scale/orchestrator";
...
describe("durable per-slot AI Bot scheduling",()=>{
  ...
  it("sanitizes a time-formatted database slot in the final BullMQ identity",()=>expect(createBullJobId(run("12:00"),"user")).toBe("ai-trade-create__user__2026-08-01__12__00__AI_BOT"));
  it("regenerates a valid deterministic ID instead of reusing a legacy creation bullJobId",()=>{const job={kind:"CREATE",userId:"user",bullJobId:"CREATE:user:2026-08-01:3:AI_BOT",jobKey:"CREATE:user:2026-08-01:3:AI_BOT",sessionRunId:"run-3",sessionRun:{localTradingDate:"2026-08-01",localSessionSlot:"3",tradeType:"AI_BOT"}},first=resolveRecoveryBullJobId(job),retry=resolveRecoveryBullJobId(job);expect(first).toEqual({jobId:"ai-trade-create__user__2026-08-01__3__AI_BOT",source:"createBullJobId()"});expect(retry).toEqual(first);expect(first.jobId).not.toContain(":")});
  it("uses the exact colon-free 8 PM creation identity",()=>expect(createBullJobId(run("3"),"user")).toBe("ai-trade-create__user__2026-08-01__3__AI_BOT"));
  ...
  it("Redis restart recovery uses persisted queued jobs and regenerated valid IDs",()=>{const orchestrator=source("lib/ai-trade-scale/orchestrator.ts");expect(orchestrator).toContain('where:{status:"QUEUED"}');expect(orchestrator).toContain("resolveRecoveryBullJobId(job)");expect(orchestrator).toContain("jobId:resolved.jobId");expect(orchestrator).toContain("boundedQueueOperation")});
  it("has no colon-bearing generated custom ID in an orchestrator-reachable add call",()=>{const orchestrator=source("lib/ai-trade-scale/orchestrator.ts"),worker=source("lib/ai-trade-scale/worker.ts");expect(orchestrator).toContain("createBullJobId");expect(orchestrator).toContain("settlementBullJobId");expect(createBullJobId(run("1"),"user")).not.toContain(":");expect(settlementBullJobId("trade")).not.toContain(":");expect(worker).toContain('deadLetterQueue.add("dead-letter"');expect(worker).not.toContain("deadLetterBullJobId")});
  it("runtime diagnostics report database and BullMQ identities separately",()=>{const diagnostic=source("scripts/ai-trade-runtime-diagnose.ts");expect(diagnostic).toContain("creationJobKey(run,user.id)");expect(diagnostic).toContain("expectedBullJobId:run?createBullJobId(run,user.id)")});
