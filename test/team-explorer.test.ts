...
describe("recursive team explorer",()=>{
  ...
  it("excludes a leaf member's own principal from Total Business",async()=>{const wallet=cloneWalletSeed();wallet.wallets.future.balance=100;wallet.rankAccount.currentStar=1;wallet.transactions=[{id:"p",userId:"child-id",wallet:"future",type:"SPOT_TO_FUTURE_TRANSFER",title:"Transfer",amount:100,balanceBefore:0,balanceAfter:100,status:"COMPLETED",reference:"wallet-transfer:p",timestamp:1}];mocks.raw.mockResolvedValueOnce([{id:"root-id",uid:"BV100000",name:"Root"}]).mockResolvedValueOnce([{count:1}]).mockResolvedValueOnce([{id:"child-id",uid:"BV100001",name:"Child",email:"child@example.com",createdAt:new Date(),wallet}]).mockResolvedValueOnce([{rootId:"root-id",totalTeam:1,qualifiedTeam:1,directCount:1,newThisMonth:1,totalBusiness:100,personalBusiness:0},{rootId:"child-id",totalTeam:0,qualifiedTeam:0,directCount:0,newThisMonth:0,totalBusiness:0,personalBusiness:100}]);const response=await GET(new Request("http://localhost/api/team?page=1&pageSize=20&search=child")),body=await response.json();expect(response.status).toBe(200);expect(body.members[0]).toMatchObject({uid:"BV100001",futureWalletBalance:100,totalTeam:0,totalBusiness:0,qualified:true});expect(body.pagination).toMatchObject({page:1,pageSize:20,total:1})});
